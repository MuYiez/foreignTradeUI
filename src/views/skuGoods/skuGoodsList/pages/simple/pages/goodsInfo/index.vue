<template>
  <div>
    ...
    <div class="form-wrap">
      ...
      <a-form-item label="商品规格" name="sku_rule" v-if="formState.spu">
        <a-input
          v-model:value="formState.sku_rule"
          allow-clear
          placeholder="支持字母 数字和英文字母"
          @change="createSku"
        />
      </a-form-item>
...
<script lang="ts">
  ...
  import { addPictrue, delPictrue, addNetPictrue, getSpuCode } from '/@/api/skuGoods/goodsInfo';
      ...
      watch(
        ...
        (newValue, oldValue) => {
          ...
          newValue[0].forEach((res: any) => {
            ...
            let file = {
              uid: Date.now() + 'order' + res.id,
              name: Date.now() + 'order' + res.id + '.png',
              status: 'done',
              url: res.url,
            };
